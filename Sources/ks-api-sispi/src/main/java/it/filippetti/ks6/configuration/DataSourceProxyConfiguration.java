...
public class DataSourceProxyConfiguration implements BeanPostProcessor {
	...
	/**
	 * Instead of directly returning a less specific datasource bean (e.g.:
	 * HikariDataSource -> DataSource), return a proxy object. See following links
	 * for why:
	...
	private static class ProxyDataSourceInterceptor implements MethodInterceptor {
		...
		public ProxyDataSourceInterceptor(DataSource dataSource, String beanName) {
			...
			this.dataSource = ProxyDataSourceBuilder.create(dataSource).name(beanName).multiline().listener(queryLogger)
					.listener(slowQueryLogger).build();
		}
		...
		public Object invoke(MethodInvocation invocation) throws Throwable {
			...
			method = ReflectionUtils.findMethod(dataSource.getClass(), invocation.getMethod().getName());
			if (method != null) {
