...
import it.filippetti.ks6.exception.ValidationException;
...
public class Fetcher {
	...
	public String toString() {
		return map.entrySet().stream().map(e -> String.format("%s{%s}", e.getKey(), e.getValue().toString()))
	...
	public static Fetcher of(Class<?> type, String fetch) throws ValidationException {
		...
		if (fetch.equals("*") && type.getAnnotation(Fetchables.class) != null) {
			fetch = Arrays.stream(type.getAnnotation(Fetchables.class).value()).map(f -> f.property())
					.collect(Collectors.joining(","));
		}
		fetch += ",";
		fetcher = new Fetcher();
		...
		while (i < fetch.length()) {
			...
			if (c != ',' && c != '(' && c != ')') {
				...
				if (c == ')' || c == '(' && ((j = indexOfRParen(fetch, i)) == -1 || property.startsWith("@"))
						|| StringUtils.isBlank(property)) {
					throw new ValidationException("fetch: invalid format");
				}
				if (!(fetchables(type).containsKey(property) || property.matches("@.+"))) {
					throw new ValidationException(String.format("fetch: invalid property '%s', allowed values are %s",
							property, fetchables(type).keySet().stream().sorted().collect(Collectors.toList())));
				}
				if (c == ',') {
				...
				if (c == '(') {
					nestedFetcher = of(fetchables(type).get(property).type(), fetch.substring(i + 1, j));
					i = j;
				}
			}
			i++;
