...
public class DualProtocolSaajSoapMessageFactory implements SoapMessageFactory, InitializingBean {
    ...
    public final static ThreadLocal<SaajSoapMessage> soapMessage = new ThreadLocal<>();
    public final static ThreadLocal<Boolean> forceJaxb = new ThreadLocal<>();
            ...
            try {
                if (SaajUtils.getSaajVersion() >= 2) {
                    if (!StringUtils.hasLength(this.messageFactoryProtocol)) {
                        ...
                        if (SessionUtils.getRequest().getRequestURI().endsWith("Soap11Endpoint/"))
                            this.messageFactoryProtocol = "SOAP 1.1 Protocol";
                        else
                            this.messageFactoryProtocol = "SOAP 1.2 Protocol";
                    }
    ...
    public SaajSoapMessage createWebServiceMessage() {
        try {
            ...
            SaajSoapMessage message = new SaajSoapMessage(saajMessage, this.langAttributeOnSoap11FaultString, this.messageFactory);
            soapMessage.set(message);
            return message;
        ...
        try {
            ...
            if (contentType.contains("startinfo")) {
                ...
                try {
                    ...
                    this.postProcess(saajMessage);
                    SaajSoapMessage message =  new SaajSoapMessage(saajMessage, this.langAttributeOnSoap11FaultString);
                    soapMessage.set(message);
                    return message;
