...
solrAdminApp.controller('DataImportController',
    function($scope, $rootScope, $routeParams, $location, $timeout, $interval, $cookies, Mbeans, DataImport, Constants) {
                ...
                DataImport.config({core: $routeParams.core, name: $scope.handler}, function (data) {
                    try {
                        ...
                        $('document > entity', xml).each(function (i, element) {
                            ...
                            //var custom_parameters = $(element).attr('parameters');
                            var info = $(element).children("info").text().trim();

                            $scope.entities.push(name);
                            if (info)
                                $scope.placeholders[name] = info;
                        });
                        $scope.refreshStatus();
                    } catch (err) {
                        console.log(err);
        ...
        $scope.onentity = function() {
            $scope.form.custom = $scope.placeholders[$scope.form.entity];
        }
        ...
        $scope.submit = function () {
            ...
            if (params.custom.length) {
                ...
                for (var i in customParams) {
                    if (customParams[i].startsWith("//") || !customParams[i])
                        continue;
        ...
        $scope.refreshStatus = function () {
            ...
            DataImport.status({core: $routeParams.core, name: $scope.handler}, function (data) {
                ...
                if (data.status != 'busy' && $scope.autorefresh) {
                    $scope.updateAutoRefresh();
                }
                ...
                if (data.status == 'busy') {
                    ...
                    $scope.info = showInfo(messages, true, 'Last Import aborted');
