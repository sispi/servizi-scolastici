package it.kdm.docer.providers.solr.bl.sicurezza;
...
//            group = groupId+"@group";
    ...
    public void addGroupsToUser(ILoggedUserInfo currentUser, String userId, List<String> newGroups) throws DocerException {
        ...
        Collection<Object> actualGroups = user.getFieldValues(Fields.User.GROUPS);
        ...
        if (actualGroups==null)
            actualGroups = new ArrayList<Object>();

        Map<String,String> userGroup = getUserOrGroup(currentUser,newGroups);
...
//                group = g+"@group";
            ...
            if (group!=null && !actualGroups.contains(group)) {
                actualGroups.add(group);
            }
        }

        metadata.addField(Fields.User.GROUPS, actualGroups);
...
//        metadata.addField(Fields.User.COD_ENTE, currentUser.getCodiceEnte());
        ...
        if (!"system".equals(currentUser.getCodiceEnte()))
...
//        metadata.addField(Fields.Group.COD_ENTE, currentUser.getCodiceEnte());
        ...
        if (!"system".equals(currentUser.getCodiceEnte())
                && !"everyone".equals(groupNewInfo.getGroupId())
                && !"admins".equals(groupNewInfo.getGroupId()))
        ...
        if (!isNullOrEmpty(groupNewInfo.getParentGroupId())) {
            ...
            if ("ente".equals(type) || "aoo".equals(type)){
                //metadata.setField(Fields.Group.PARENT_GROUP_ID, doc.getFieldValue("sid"));
                //metadata.removeField(Fields.Group.PARENT_GROUP_ID);
                metadata.setField(Fields.Group.PARENT_GROUP_ID,(String)doc.get("GROUP_ID"));
                if (type.equals("ente")){
                    metadata.setField(Fields.Group.COD_ENTE, doc.getFieldValue("sid"));
                } else if (type.equals("aoo")) {
                    metadata.setField(Fields.Group.COD_ENTE, doc.getFieldValue("COD_ENTE"));
                    metadata.setField(Fields.Group.COD_AOO, doc.getFieldValue("sid"));
                }
            } else {
                String COD_AOO = (String) doc.get("COD_AOO");
                if (COD_AOO!=null)
                    metadata.setField(Fields.Group.COD_AOO, COD_AOO);
            }
        }
