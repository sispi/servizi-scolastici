package it.kdm.docer.providers.solr.bl.documenti;
...
public class DocumentoBL extends SolrBaseUtil {
    ...
    private String manageVersion(ILoggedUserInfo currentUser, String docId, InputStream documentFile, Boolean isNewVersion) throws DocerException {
        ...
        returnProps.add(Fields.Documento.COD_AOO);
        returnProps.add(Fields.Documento.VERSIONS);
        returnProps.add(Fields.Documento.DOCNAME);
        returnProps.add(Fields.Documento.XML_VERSIONS);
        ...
        String docname = (String)solrDoc.getFieldValue(Fields.Documento.DOCNAME);
        String VERSIONS = (String) solrDoc.getFieldValue(Fields.Documento.XML_VERSIONS);

        Collection<Object> versions = solrDoc.getFieldValues(Fields.Documento.VERSIONS);

        List<String> l = convertSolrCollectionToMap(versions);

        int nextVersion = l.size();
        ...
        int idx = VERSIONS.lastIndexOf("<");

        if (!isNewVersion){
            Pattern pattern = Pattern.compile("<version><number>"+nextVersion+"</number>");
        ...
        String now = SolrBaseUtil.simpleDateFormat.format(new Date());

        String NEW = "<version><number>"+nextVersion+"</number><date>"+now+"</date><userId>"+currentUser.getUserId()+"</userId><filename>"+docname+"</filename></version>";

        VERSIONS = VERSIONS.substring(0,idx) + NEW + "</versions>";
...
//        metadata.addField(Fields.Documento.SOLR_ID,solrId);
        ...
        metadata.addField(Fields.Documento.XML_VERSIONS,VERSIONS);
    ...
    public InputStream streamLastVersion(ILoggedUserInfo currentUser, String docId) throws DocerException {
        return streamVersion(currentUser,docId,"last");
    }

    public InputStream streamVersion(ILoggedUserInfo currentUser, String docId, String versionNumber) throws DocerException {
        try {
    ...
    public ContentStreamBase downloadFile(ILoggedUserInfo currentUser, String docId, String version) throws DocerException {
        List<String> returnProps = new ArrayList<String>();
        returnProps.add(Fields.Documento.SOLR_ID);
        ...
        Long size = ((Number)solrDoc.getFieldValue(Fields.Documento.CONTENT_SIZE)).longValue();

        ContentStreamBase stream = getSolrStream(currentUser,solrId,version,size);
    ...
    public String createDocument(ILoggedUserInfo currentUser, String documentType, Map<String, String> profileInfo, InputStream documentFile) throws DocerException {
        ...
        String docname = (String) metadata.get("DOCNAME").getValue();

        String VERSIONS = "<versions><version><number>1</number><date>"+now+"</date><userId>"+currentUser.getUserId()+"</userId><filename>"+docname+"</filename></version></versions>";

        metadata.setField(Fields.Documento.XML_VERSIONS, VERSIONS);
...
//            newList.add(getSolrId(i, metaDati));
    ...
    public void lockDocument(ILoggedUserInfo currentUser, String docId) throws DocerException {
        ...
        long duration = DEFAULT_LOCK_DURATION;

        if (extra!=null && extra.containsKey("expiration")){
            duration = (Long) extra.get("expiration");
        }
