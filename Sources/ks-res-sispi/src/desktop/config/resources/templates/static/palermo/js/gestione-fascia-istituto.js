...
function caricaElencoTariffe(){
	...
	$.ajax ({
		method: 'GET',
		//url: '/bpm-server/db/select/PORTSCU_SCU_ASILOTARIFFE',
		url: '/sispi/v1/asilotariffe/lista',
		...
		success: function(data, textStatus, jqXHR){
			if(data.length==0){
				...
				for (i = 0; i < data.length; i++) {
					...
					var descrizioneCategoria = dataItem.descrizioneCategoria;
		...
		error: function(jqXHR, textStatus, errorThrown) {
			console.log('Errore nel caricamento SCU_ASILOTARIFFE');
			visualizzaMessaggio('validazione-div', '<p>Errore nel caricamento dati SCU_ASILOTARIFFE</p>', 'danger');
		},
