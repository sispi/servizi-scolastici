    ...
    <p:choose href="#xformed-data">
        ...
        <p:when test="/xhtml:html">
            ...
            <p:choose href="#request">
                <p:when test="/request/parameters/parameter[name = 'orbeon-theme']/value = 'xforms' ">
                    <!-- Plain theme -->
                    <p:processor name="oxf:identity">
                        <p:input name="data">
                            <config>oxf:/config/frontend-xforms.xsl</config>
                        </p:input>
                        <p:output name="data" id="theme-config"/>
                    </p:processor>
                </p:when>
                <p:when test="starts-with(/request/request-path, '/frontend-')">
                    <!-- Plain theme -->
                    <p:processor name="oxf:identity">
                        <p:input name="data" transform="oxf:xslt" href="#request" >
                            <config xsl:version="2.0" >
                            oxf:/config/<xsl:value-of select="tokenize(/request/request-path, '/')[2]"/>.xsl
                            </config>
                        </p:input>
                        <p:output name="data" id="theme-config"/>
                    </p:processor>
