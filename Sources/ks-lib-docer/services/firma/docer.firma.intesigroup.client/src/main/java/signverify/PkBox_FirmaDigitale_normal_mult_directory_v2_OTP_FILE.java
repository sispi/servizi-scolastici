...
public class PkBox_FirmaDigitale_normal_mult_directory_v2_OTP_FILE {
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		try {
			...
			if (file != null && file.isDirectory() && file.canRead()) {
				...
				if (ultimo_carattere.equals("\\")&& ultimo_carattere_new_dir.equals("\\"))percorso_completo_new_dir = (dir + dir_new);
				...
				if (success_new_dir) {
					...
					System.out.print("INSERIRE PIN DEL FIRMATARIO: ");
					pin = new BufferedReader(new InputStreamReader(System.in));
					pin_num = pin.readLine();
					
					for (int i = 0; i < as.length; i++) {
						
						System.out.print("INSERIRE OTP DEL FIRMATARIO: ");
					...
					if (firm.toUpperCase().equals("S")) {
						System.out.print("INSERIRE ALIAS DEL FIRMATARIO: ");
						alias = new BufferedReader(new InputStreamReader(System.in));
						alias_name = alias.readLine();
						...
						for (int i = 0; i < array_elenco_file_new_dir.length; i++) {
							...
							enve.addsign(null, fileIn, null, alias_name, pin_num, otp_num, Envelope.base64Encoding, null, fileOut);
