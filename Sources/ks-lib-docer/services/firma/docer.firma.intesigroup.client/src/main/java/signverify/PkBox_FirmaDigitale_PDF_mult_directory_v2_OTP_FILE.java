...
public class PkBox_FirmaDigitale_PDF_mult_directory_v2_OTP_FILE {
	/**
	 * @param args
	 */
				...
				if (success_new_dir) {
					...
					for (int i = 0; i < as.length; i++) {
						...
						if (estensione_file.toUpperCase().equals("PDF")) {
							...
							docfile_Out = percorso_completo_new_dir + as[i].substring(0,((as[i].length()) - 4))+ "_sign.pdf";
					...
					if (firm.toUpperCase().equals("S")) {
						...
						for (int j = 0; j < array_elenco_file_new_dir.length; j++) {
							percorso_new_file = percorso_completo_new_dir+ array_elenco_file_new_dir[j];
