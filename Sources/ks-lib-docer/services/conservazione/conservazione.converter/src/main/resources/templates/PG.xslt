...
<xsl:stylesheet version="2.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:utils="java:it.kdm.docer.conservazione.Utils">
	...
	<xsl:template match="/">
		<unitaDocumentaria>
			...
			<xsl:if test="utils:isDateParsable($data)">
				<data>
					<xsl:value-of select="utils:parseDateTime($data)" />
				</data>
			</xsl:if>
	...
	<xsl:template name="documento-template">
		...
		<xsl:if test="$mode = 'DOC'">
			<datiSpecifici>
				...
				<xsl:if test="MS_T_REGISTRAZ and MS_T_REGISTRAZ != ''">
					<tipo_Registrazione>
						<xsl:value-of select="MS_T_REGISTRAZ" />
					</tipo_Registrazione>
				</xsl:if>
			</datiSpecifici>
		</xsl:if>
		...
		<componente>
			<xsl:variable name="filename">
				<xsl:choose>
					<xsl:when test="DOCNUM and DOCNUM != ''">
						<xsl:value-of
							select="concat(DOCNUM, '-', DOCNAME, '.', DEFAULT_EXTENSION)" />
					</xsl:when>
					<xsl:otherwise>
			...
			<formatoFileVersato>
				<xsl:value-of select="utils:findExtension($filename)" />
			</formatoFileVersato>
			...
			<xsl:if test="utils:isDateParsable(D_CO_CER)">
				<riferimentoTemporale>
					<xsl:value-of select="utils:parseDateTime(D_CO_CER)" />
				</riferimentoTemporale>
				<descrizioneRiferimentoTemporale>
					<xsl:value-of select="T_D_CONTR_CER" />
				</descrizioneRiferimentoTemporale>
			</xsl:if>
		</componente>
	</xsl:template>
