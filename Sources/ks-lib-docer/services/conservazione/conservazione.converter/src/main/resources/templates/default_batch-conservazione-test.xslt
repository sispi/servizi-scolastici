...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/">
		<unitaDocumentaria>
			...
			<!-- TODO: uncomment <codiceEnte><xsl:value-of select="/root/documento/COD_ENTE" 
				/></codiceEnte> <codiceAoo><xsl:value-of select="/root/documento/COD_AOO" 
				/></codiceAoo> -->
			<codiceEnte>ente_test</codiceEnte>
			<codiceAoo>KDM_TEST</codiceAoo>

			<ambiente>PARER_TEST</ambiente>

			<!-- TODO: uncomment <userId><xsl:value-of select="/root/documento/AUTHOR_ID" 
				/></userId> -->
			<userId>KDM_Versatore</userId>
			<numeroPG>
				<xsl:value-of select="/root/documento/NUM_PG" />
			</numeroPG>
			<annoPG>
				<xsl:value-of select="/root/documento/ANNO_PG" />
			</annoPG>
			<registroPG>
				<xsl:value-of select="/root/documento/REGISTRO_PG" />
			</registroPG>

			<xsl:variable name="type">
				<xsl:text>GENERICO</xsl:text>
			</xsl:variable>
			<tipologia>
			...
			<oggetto>
				<xsl:value-of select="/root/documento/OGGETTO_PG" />
			</oggetto>
			<data>
				<xsl:value-of select="/root/documento/D_REGISTRAZ" />
			</data>
	...
	<xsl:template name="documento-template">
		...
		<datiSpecifici>
			...
			<AttributoStringa>abc</AttributoStringa>
			<AttributoData>
		...
		<componente>
			<xsl:variable name="filename"
				select="concat(DOCNAME, '.', DEFAULT_EXTENSION)" />
			<id>
			...
			<xsl:choose>
				<xsl:when test="USA_D_CO_CER = 'SI'">
					...
					<riferimentoTemporale>
						<xsl:value-of select="MS_D_CO_CER" />
					</riferimentoTemporale>
					<descrizioneRiferimentoTemporale>
						<xsl:value-of select="MS_T_D_CONTR_CER" />
					</descrizioneRiferimentoTemporale>
				</xsl:when>
				<xsl:otherwise>
