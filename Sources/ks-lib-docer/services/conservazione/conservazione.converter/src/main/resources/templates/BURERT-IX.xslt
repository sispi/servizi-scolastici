	...
	<xsl:template match="/">
		<unitaDocumentaria>
			...
			<xsl:variable name="type">
				<xsl:text>BOLLETTINO UFFICIALE</xsl:text>
			</xsl:variable>			
	...
	<xsl:template name="documento-template">
		...
		<xsl:if test="$mode = 'DOC'">
			<datiSpecifici>
				...
				<xsl:if test="ANNATA_BU and ANNATA_BU != ''" >
					<annata><xsl:value-of select="ANNATA_BU" /></annata>
				</xsl:if>
				<xsl:if test="DATA_BU and DATA_BU != ''" >
					<data><xsl:value-of select="DATA_BU" /></data>
				</xsl:if>
				<xsl:if test="NUMERAZIONE_BU and NUMERAZIONE_BU != ''" >
					<numerazione><xsl:value-of select="NUMERAZIONE_BU" /></numerazione>
				</xsl:if>
				<xsl:if test="NUMERO_BU and NUMERO_BU != ''" >
					<numero><xsl:value-of select="NUMERO_BU" /></numero>
				</xsl:if>
				<xsl:if test="PARTE_BU and PARTE_BU != ''" >
					<parte><xsl:value-of select="PARTE_BU" /></parte>
				</xsl:if>
			</datiSpecifici>
		</xsl:if>
