...
public class ConfigurationManager {
	...
	public void putLoginInfo(String token, LoginInfo loginInfo) throws SQLException {
		...
		if (logInfo==null)
			jdbcTemplate.update("insert into loginInfo (token,loginInfo,insertDate) values (?,?,sysdate)", token, enc);
		else
			jdbcTemplate.update("update loginInfo set loginInfo=?, insertDate=sysdate where token=? ", enc, token);
	}
	...
	public void putWSTicket(WSTicketAuthInfo wsTicketAuthInfo, String ticket) throws SQLException {
		...
		if (Strings.isNullOrEmpty(tic))
			jdbcTemplate.update("insert into ticketsessions (authInfo,ticket,insertDate) values (?,?,sysdate)", wsTicketAuthInfo.hashCode(), enctic);
		else
			jdbcTemplate.update("update ticketsessions set authInfo=?, insertDate=sysdate where ticket=? ", wsTicketAuthInfo.hashCode(), enctic);
	}
...
//Mysql
	//String select = "select id, ente, idserv, groupname, author, created, xmlconfig, status, lasterror" + " from servicesconfig" + " where" + " idserv = ? and ucase(ente)=ucase(?)"
	//	+ " order by id desc limit 1";
//Oracle
        String select = "select id, ente, idserv, groupname, author, created, xmlconfig, status, lasterror" + " from servicesconfig" + " where" + " idserv = ? and UPPER(ente)=UPPER(?) and rownum=1"
                + " order by id desc";
...
//Oracle
		Integer idserv = Integer.parseInt(service.get("id").toString());
        ...
        Number status = (Number) serviceConfig.get("status");
...
//Oracle
        String select = "select id, ente, idserv, groupname, author, created, xmlconfig, status, lasterror" + " from servicesconfig" + " where" + " idserv = ? and UPPER(ente)=UPPER(?)"
        	+ " and rownum<=15 order by created desc";
...
//Mysql
	   // String select = "select xmlconfig" + " from servicesconfig" + " where" + " idserv = ? and ucase(ente)=ucase(?)" + " order by id desc limit 1";
//Oracle
        String select = "select xmlconfig" + " from servicesconfig" + " where" + " idserv = ? and UPPER(ente)=UPPER(?)" + " and rownum=1 order by id desc";
	    List<Map<String, Object>> servicesConfigurations = jdbcTemplate.queryForList(select, idserv, ente);
	...
	// services e servicesconfig
        ...
        //Mysql
        //List<Map<String, Object>> serviceConfigByEnte = jdbcTemplate.queryForList("select id from servicesconfig where idserv=? and ucase(ente)=ucase(?) order by id desc limit 1", id, ente);
//Oracle
        List<Map<String, Object>> serviceConfigByEnte = jdbcTemplate.queryForList("select id from servicesconfig where idserv=? and UPPER(ente)=UPPER(?) and rownum=1 order by id desc", id, ente);
...
//Mysql
			//serviceConfigByEnte = jdbcTemplate.queryForList("select id from servicesconfig where idserv=? and ucase(ente)=ucase(?) order by id desc limit 1", id, ente);
//Oracle
            serviceConfigByEnte = jdbcTemplate.queryForList("select id from servicesconfig where idserv=? and UPPER(ente)=UPPER(?) and rownum=1 order by id desc", id, ente);
			for (Map<String, Object> sc : serviceConfigByEnte) {
			    if (sc.get("id") != null) {
				idServiceByEnte = String.valueOf(sc.get("id"));
			    }
