...
public class ConfigurationManager {
	...
	public void putLoginInfo(String token, LoginInfo loginInfo) throws SQLException {
		...
		if (logInfo==null)
			jdbcTemplate.update("insert into loginInfo (token,loginInfo,insertDate) values (?,?,NOW())", token, enc);
		else
			jdbcTemplate.update("update loginInfo set loginInfo=?, insertDate=NOW() where token=? ", enc, token);
	}
	...
	public void putWSTicket(WSTicketAuthInfo wsTicketAuthInfo, String ticket) throws SQLException {
		...
		if (Strings.isNullOrEmpty(tic))
			jdbcTemplate.update("insert into ticketsessions (authInfo,ticket,insertDate) values (?,?,NOW())", wsTicketAuthInfo.hashCode(), enctic);
		else
			jdbcTemplate.update("update ticketsessions set authInfo=?, insertDate=NOW() where ticket=? ", wsTicketAuthInfo.hashCode(), enctic);
	}
